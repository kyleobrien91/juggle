...
from juggle.apps.accounts.exceptions import (
    InvalidPasswordError,
    InvalidResetPasswordSignatureError,
    WrongPasswordError,
)
from juggle.apps.accounts.models import UserAccount
from juggle.apps.accounts.services.password import PasswordService


PASSWORD_SERVICE_PATH = "juggle.apps.accounts.services.password.PasswordService"  # nosec
...
def test_reset_password_signature_failure(mocker, settings, exception):
    ...
    settings.JUGGLE_RESET_PASSWORD_EXPIRATION_DELTA = 42
    mocked_change_password = mocker.patch(f"{PASSWORD_SERVICE_PATH}.change_password")
